...
def test_step_prompt_extracts_acceptance_criteria_from_github_issue_drafts(
    py_sandbox: LocalSubprocessSandbox,
    fake_llm_factory: Callable[[list[Any]], OpenAIClient],
) -> None:
    canned = [
    ...
    assignment.inputs["requirement_analysis"] = {
        "summary": "Add greet(name)",
        "issue_drafts": [
            {
                "title": "Add greet utility",
                "acceptance_criteria": ["GH-AC: greet('world') returns hello"],
            }
        ],
    }
    ...
    first_call = llm._client.chat.completions.calls[0]  # type: ignore[attr-defined]
    user_msg = first_call["messages"][1]["content"]
    assert "GH-AC: greet('world') returns hello" in user_msg
