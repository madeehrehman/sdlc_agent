...
def test_backlog_analyzer_records_one_event(
    ...
    assert e["metadata"]["schema_name"] == "github_backlog_analyzer_response"
    assert e["metadata"]["source_spec"] == "specs.md"
...
def test_recorder_optional_subagent_runs_without_it(
    ...
    llm = fake_llm_factory([_backlog_canned()])
    analyzer = BacklogAnalyzer(llm=llm, github=github_project)
    out = analyzer.run(
        TaskAssignment(
            task_id="t1",
            ticket_id="GH-1",
