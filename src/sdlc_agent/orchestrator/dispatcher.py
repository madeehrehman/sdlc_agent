...
from sdlc_agent.mcp.github import GitHubProjectClient, GitHubProjectError
...
_DEFAULT_PERMISSIONS: dict[SubagentName, Permissions] = {
    SubagentName.BACKLOG_ANALYZER: Permissions(filesystem="read-only", git="none", github="project"),
    SubagentName.DEVELOPER: Permissions(filesystem="read-write", git="read", github="issue"),
    SubagentName.PR_REVIEWER: Permissions(filesystem="read-only", git="review", github="pr"),
}
...
class Orchestrator:
    ...
    def intake(
        ...
        *,
        ...
        (GitHub issue/project ids, git refs, etc.) reaches the subagents.
        """
    ...
    def _evaluate_gate(self, state: TicketState, gate: SDLCPhase) -> None:
        ...
        if gate is SDLCPhase.REQUIREMENTS_GATE and decision is GateDecision.PROCEED:
            self._adopt_created_github_issue(state, artifact)
    ...
    # ------------------------------------------------------- GitHub lifecycle
    def _sync_github_lifecycle(self, state: TicketState, phase: SDLCPhase) -> None:
        if self.github is None:
            return
        item_id = state.ticket_inputs.get("github_project_item_id")
        issue_number = state.ticket_inputs.get("github_issue_number")
        if not item_id:
            return

        done_status = (
            "Done" if state.ticket_inputs.get("release_to_main_accepted") else "Release Ready"
        )
        status_by_phase = {
            SDLCPhase.DEVELOPMENT: "In Development",
            SDLCPhase.PR_REVIEW: "In Review",
            SDLCPhase.DONE: done_status,
            SDLCPhase.BLOCKED: "Blocked",
            SDLCPhase.NEEDS_HUMAN: "Needs Human",
        }
        status = status_by_phase.get(phase)
        if status is None:
            return
        try:
            self.github.update_project_status(str(item_id), status)
            if (
                phase is SDLCPhase.DONE
                and issue_number is not None
                and state.ticket_inputs.get("release_to_main_accepted")
            ):
                self.github.close_issue(int(issue_number))
        except (GitHubProjectError, ValueError) as e:
            self._log_episode(
                "github_sync_failed",
                state,
                extra={"phase": phase.value, "error": str(e)},
            )

    def _adopt_created_github_issue(
        self, state: TicketState, artifact: ArtifactReturn
    ) -> None:
        if state.ticket_inputs.get("github_issue_number") and state.ticket_inputs.get(
            "github_project_item_id"
        ):
            return
        created = artifact.artifact.get("created_issues")
        ...
        item_id = first.get("project_item_id")
        if issue_number is None or item_id is None:
            return
        state.ticket_inputs["github_issue_number"] = issue_number
        state.ticket_inputs["github_project_item_id"] = item_id
