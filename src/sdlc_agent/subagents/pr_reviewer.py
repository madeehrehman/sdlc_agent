...
Your job:
  ...
  * Review the diff against the requirement analysis and acceptance criteria.
  * Use the implementation summary as a claim to verify, not as ground truth.
...
Constraints:
  ...
  * If acceptance criteria are present and the diff does not satisfy them,
    request changes.
...
class PRReviewer:
    ...
    def _build_user_prompt(
        ...
        *,
        ...
        requirement_analysis = assignment.inputs.get("requirement_analysis") or {}
        implementation_summary = assignment.inputs.get("implementation_summary") or {}
        ac: list[str] = []
        if isinstance(requirement_analysis, dict):
            ac = PRReviewer._acceptance_criteria(requirement_analysis)
        impl_summary = (
            implementation_summary.get("implementation_summary", "")
            if isinstance(implementation_summary, dict)
            else ""
        )
        impl_tests = (
            implementation_summary.get("test_files", [])
            if isinstance(implementation_summary, dict)
            else []
        )
        files = "\n".join(f"  - {f}" for f in files_changed) or "  (none)"
        ac_text = "\n".join(f"  - {item}" for item in ac) or "  (none provided)"
        test_text = "\n".join(f"  - {item}" for item in impl_tests) or "  (none claimed)"
        trunc_note = (
...
Acceptance criteria from requirement analysis:
{ac_text}

Implementation summary to verify:
  {impl_summary or "(none provided)"}
