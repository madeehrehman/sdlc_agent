"""DeveloperTester subagent (spec §3.2, §12).

A stateless TDD loop: the LLM proposes one step at a time (write_test /
write_code / run_tests / complete); the DeveloperTester applies it in its sandbox and
...
_STEP_SYSTEM_PROMPT = """\
You are the DeveloperTester subagent. You implement against a GitHub Issue's
requirement analysis, strictly test-first: the SAME loop that writes the code
writes its unit and integration tests.
...
Discipline:
  ...
  * Prefer both unit-level and integration-level coverage when the acceptance
    criterion crosses module boundaries.
...
You are the DeveloperTester subagent producing the final return artifact after a
TDD loop. Be honest about what you wrote and whether tests are green.
...
under `impl_files`. The orchestrator will use this artifact as evidence for a
feature PR targeting `develop`. Optionally propose durable memory with concrete
evidence drawn from the loop trace (e.g. "this project uses unittest, not
pytest").
...
@dataclass
class Developer:
    """Stateless TDD-loop DeveloperTester (spec §3.2 / §12 merged-loop tradeoff)."""
    ...
    def _build_step_prompt(
        *,
        ...
        ac = Developer._acceptance_criteria(ra)
...
"""
    ...
    @staticmethod
    def _acceptance_criteria(requirement_analysis: dict[str, Any]) -> list[str]:
        direct = requirement_analysis.get("acceptance_criteria")
        if isinstance(direct, list) and direct:
            return [str(item) for item in direct]

        out: list[str] = []
        for key in ("created_issues", "issue_drafts"):
            items = requirement_analysis.get(key)
