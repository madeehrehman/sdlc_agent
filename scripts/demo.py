...
Drives a non-trivial GitHub-native work item (`GH-42` — a TokenBucket rate limiter)
...
from sdlc_agent.mcp.github import FixtureGitHubProject  # noqa: E402
...
_IMPL_FILE = '''\
"""Pure-Python token-bucket rate limiter (GH-42)."""
...
def _backlog_response() -> dict[str, Any]:
    return {
        "artifact": {
            ...
            "repo_gaps": ["TokenBucket helper is specified but not implemented"],
            "issue_drafts": [
                {
                    "title": "Add TokenBucket rate limiter helper",
                    "body": "Implement TokenBucket(capacity, refill_per_second).allow(key).",
                    "acceptance_criteria": [
            ...
            "notes": "specs.md is clear enough to seed a GitHub issue.",
        },
        "proposed_memory": [
            {
                "scope": "project_fact",
                "claim": "this project's test runner is unittest (not pytest)",
                "evidence": "specs.md requests deterministic unittest coverage with injected now()",
                "confidence": "high",
            }
        ],
    }
...
def _init_sample_git_repo(parent: Path) -> dict[str, Any]:
    ...
    git("commit", "-q", "-m", "feat(GH-42): add TokenBucket helper")
...
def _write_specs(target_repo: Path) -> None:
    (target_repo / "specs.md").write_text(
        "# Product spec\n\n"
        "## Token bucket rate limiter\n\n"
        "Build a pure-Python TokenBucket(capacity, refill_per_second).allow(key) helper.\n\n"
        "Acceptance criteria:\n"
        "- First N calls for a key are allowed, then calls are denied until refill.\n"
        "- Refill happens at refill_per_second tokens/sec and clamps to capacity.\n"
        "- Tests use an injected now() clock and do not sleep.\n",
        encoding="utf-8",
    )
...
def _run_demo(target_repo: Path, *, quiet: bool) -> int:
    ...
    say(_box("SDLC Deep Agent -- End-to-end demo (GH-42)"))
    ...
    _write_specs(target_repo)
    say("      wrote:    specs.md (living backlog seed)")
    ...
    #   canned_llm = OpenAIClient(model=cfg.model.name)  # reads OPENAI_API_KEY from env
    # and the orchestrator + subagents will hit the real API instead.

    github = FixtureGitHubProject(repo_root=target_repo, project_name="SDLC")
    orch_session = "gh-42"
    ...
    final = orch.run_to_completion("GH-42")
    ...
    impl = stores.load_artifact("GH-42", SDLCPhase.DEVELOPMENT)
    review = stores.load_artifact("GH-42", SDLCPhase.PR_REVIEW)
    if impl is not None:
        say(
